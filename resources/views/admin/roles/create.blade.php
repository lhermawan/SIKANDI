    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <div class="flex items-center gap-2 text-xs text-slate-400 mb-1">
                <span>Administrasi</span><span>/</span><a href="{{ route('admin.roles.index') }}" class="hover:text-slate-300">Role & Akses</a><span>/</span><span class="text-white">Tambah</span>
            </div>
            <h1 class="text-2xl font-bold text-white tracking-tight">Tambah Role Baru</h1>
        </div>
    </div>

    <form action="{{ route('admin.roles.store') }}" method="POST" class="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm">
        @csrf
        <div class="p-6 border-b border-slate-800">
            ...
            <input type="text" name="name" id="name" required class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-white focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition placeholder-slate-600" placeholder="Misal: Admin Server, Satgas Siber, dll" value="{{ old('name') }}">
            @error('name')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <label class="flex items-start gap-3 p-3 rounded-xl border border-slate-800 bg-slate-950 hover:border-slate-700 cursor-pointer transition">
                    <div class="flex items-center h-5 mt-0.5">
                        <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" class="w-4 h-4 bg-slate-900 border-slate-700 rounded text-blue-600 focus:ring-blue-500/30 focus:ring-offset-slate-950">
                    </div>
        ...
        <div class="p-4 bg-slate-800/30 border-t border-slate-800 flex items-center justify-end gap-3">
            ...
            <button type="submit" class="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-semibold shadow-lg shadow-blue-600/30 transition">Simpan Role</button>
        </div>
    </form>
</div>
@endsection

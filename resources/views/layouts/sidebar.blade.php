<aside class="fixed inset-y-0 left-0 z-40 w-64 bg-slate-900/95 backdrop-blur-xl border-r border-slate-800/80 flex flex-col transition-transform duration-300 lg:translate-x-0 -translate-x-full" id="sidebar">
    ...
    <div class="h-16 flex items-center gap-3 px-5 border-b border-slate-800/80">
        <div class="w-9 h-9 rounded-xl overflow-hidden shadow-md shadow-blue-500/20">
            <img src="{{ asset('images/logo.jpg') }}" alt="SIKANDI Logo" class="w-full h-full object-cover">
        </div>
        <div>
    ...
    <div class="flex-1 overflow-y-auto px-3 py-4 space-y-5 text-xs">
        ...
        <div class="space-y-1">
            ...
            <a href="{{ route('security.public-incidents.index') }}"
                class="flex items-center justify-between px-3 py-2 rounded-xl transition {{ request()->routeIs('security.public-incidents.*') ? 'bg-slate-800 text-blue-400 font-semibold' : 'text-slate-300 hover:bg-slate-800/60 hover:text-white' }}">
                <span class="flex items-center gap-3">
                    <svg class="w-4 h-4 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>
                    <span>Insiden Publik</span>
                </span>
                @php
                    $pendingPublicCount = \App\Models\PublicIncidentReport::where('status', 'pending_review')->count();
                @endphp
                @if($pendingPublicCount > 0)
                    <span class="px-2 py-0.5 text-[10px] font-bold rounded-full bg-amber-500/20 text-amber-400 border border-amber-500/30 animate-pulse">
                        {{ $pendingPublicCount }}
                    </span>
                @endif
            </a>

            <a href="{{ route('security.threat-actors.index') }}"
                class="flex items-center gap-3 px-3 py-2 rounded-xl transition {{ request()->routeIs('security.threat-actors.*') ? 'bg-slate-800 text-blue-400 font-semibold' : 'text-slate-300 hover:bg-slate-800/60 hover:text-white' }}">
                <svg class="w-4 h-4 text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09A13.916 13.916 0 008 11a4 4 0 118 0c0 1.017-.07 2.019-.203 3m-2.118 6.844A21.88 21.88 0 0015.171 17m3.839 1.132c.645-2.266.99-4.659.99-7.132A8 8 0 008 4.07M3 15.364c.64-1.319 1-2.8 1-4.364 0-1.457.39-2.823 1.07-4"/></svg>
                <span>Threat Actors</span>
            </a>

            <a href="{{ route('security.approvals.index') }}"
                class="flex items-center justify-between px-3 py-2 rounded-xl transition {{ request()->routeIs('security.approvals.*') ? 'bg-slate-800 text-blue-400 font-semibold' : 'text-slate-300 hover:bg-slate-800/60 hover:text-white' }}">
                <span class="flex items-center gap-3">
                    <svg class="w-4 h-4 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    <span>SOC Approvals</span>
                </span>
                @php
                    $pendingCount = \App\Models\SecurityIncidentResponse::where('status', 'pending')->count();
                @endphp
                @if($pendingCount > 0)
                    <span class="px-2 py-0.5 bg-amber-500/20 text-amber-500 text-[10px] font-bold rounded-full animate-pulse">{{ $pendingCount }}</span>
                @endif
            </a>
            ...
            <a href="{{ route('security.risks.index') }}"
        ...
        <div class="space-y-1 pt-2 border-t border-slate-800/80">
            ...
            <a href="{{ route('admin.locations.index') }}"
                class="flex items-center gap-3 px-3 py-2 rounded-xl transition {{ request()->routeIs('admin.locations.*') ? 'bg-slate-800 text-blue-400 font-semibold' : 'text-slate-300 hover:bg-slate-800/60 hover:text-white' }}">
            ...
            <a href="{{ route('admin.roles.index') }}"
                class="flex items-center gap-3 px-3 py-2 rounded-xl transition {{ request()->routeIs('admin.roles.*') ? 'bg-slate-800 text-blue-400 font-semibold' : 'text-slate-300 hover:bg-slate-800/60 hover:text-white' }}">
